...
import { AlertTriangle, CheckCircle2, Clipboard, Loader2, Terminal } from "lucide-react";
          ...
          {!state.loading && !state.error && state.requirements.length > 0 && (
            <div className="mt-3 space-y-2">
              {state.requirements.map((item) => (
                <div key={item.name} className="flex flex-wrap items-center gap-2 rounded-lg border border-gray-700/70 bg-gray-950/50 px-3 py-2">
                  ...
                  {!item.ok && item.packageId && (
                    <Button type="button" size="sm" variant="secondary" loading={installing === item.packageId} onClick={() => installPackage(item)}>
                      Install
                    </Button>
                  )}
                  {!item.ok && item.installCommand && (
                    <button type="button" onClick={() => copyCommand(item.installCommand || "")} className="inline-flex items-center gap-1 rounded-md border border-gray-700 px-2 py-1 text-xs text-gray-300 hover:text-white">
                      <Clipboard className="h-3.5 w-3.5" /> Copy command
                    </button>
                  )}
                  {!item.ok && (
                    <a href={serverId ? `/servers/${encodeURIComponent(serverId)}` : "/terminal"} className="inline-flex items-center gap-1 rounded-md border border-gray-700 px-2 py-1 text-xs text-gray-300 hover:text-white">
